...
1. **mints-wind-data-ingestion** → Fetches & stores wind data in PostgreSQL.  
2. **mints-sensordata-to-postgres-backend** → Ingests calibrated sensor data into PostgreSQL.  
3. **mints-noaa-api** → Serves wind & sensor data via REST API.  
4. **sharedairdfw_map** → Fetches from the API and displays it on the public website.
